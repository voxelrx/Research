...
def init_weights(shape):
    ...
    if len(shape) == 2:
        ...
        scale_factor = (1 / np.sqrt(np.sum(w ** 2, axis=1)))
        w = w * np.tile(scale_factor.reshape((shape[0], 1)), (1, w.shape[1]))
        ...
        w = np.random.randn(*shape)
        w = w.reshape(shape[0], shape[1], shape[2] * shape[3] * shape[4])
        scale_factor = (1 / np.sqrt(np.sum(w ** 2, axis=2)))
        w = w.T * scale_factor.T
        w = w.T.reshape(shape[0], shape[1], shape[2], shape[3], shape[4])
        
    return theano.shared(np.asarray(w, dtype=theano.config.floatX))


def unshared_init_weights(shape):
