...
def rsa(representations):
    ...
    # rotate the array
    representations = representations.T  # d x n [class, neurons]

    # define units of comparison
    [category, neurons] = representations.shape

    # initialize empty RDM
    rdm = np.zeros((category, category))

    # loop over each pairwise comparison and calculate representational dissimilarity
    for i in xrange(category):
        for j in xrange(category):
...
def multidimensional_scaling(rdm, labels):
    ...
    for index, label in enumerate(labels):
        plt.annotate(label[0][0], (positions[index, 0], positions[index, 1]))
